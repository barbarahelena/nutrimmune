# Heatmap of genes of interest (baseline - one year difference)
# Barbara Verhaar
...
create_complex_heatmaps <- function(all_results) {
  ...
  dir.create("results/correlations/heatmaps_v0v4", showWarnings = FALSE, recursive = TRUE)
  ...
  clinical_vars <- c("deltaCRP", "deltaHOMA-IR", "deltaBMI")
  ...
  for (i in seq_along(tissues)) {
    ...
    # Create heatmap
    can_cluster_cols <- sum(complete.cases(t(corr_matrix))) >= 2 # can we cluster cols?
    ht <- Heatmap(
      corr_matrix,
      name = "Correlation",
      col = corr_col_fun,
    ...
    # Save the plot
    pdf(paste0("results/correlations/heatmaps_v0v4/heatmap_", tissue, ".pdf"), width = 5, height = 9)  
  ...
  pdf("results/correlations/heatmaps_v0v4/heatmap_all_tissues.pdf", width = 8, height = 25)
  draw(ht_all, annotation_legend_list = list(lgd_sig))
  dev.off()

  svg("results/correlations/heatmaps_v0v4/heatmap_all_tissues.svg", width = 8, height = 25)
  draw(ht_all, annotation_legend_list = list(lgd_sig))
  dev.off()
...
main <- function() {
  ...
  if (length(all_results) > 0) {
    ...
    cat("Heatmaps saved in results/correlations/heatmaps_v0v4/\n")
  } else {
