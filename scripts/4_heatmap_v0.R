...
create_complex_heatmaps <- function(all_results) {
  ...
  dir.create("results/correlations_macronutrients/heatmaps_v0", showWarnings = FALSE, recursive = TRUE)
  ...
  clinical_vars <- c("Total Calories", "Carbohydrates", "Fat", "Saturated Fat", "Protein", "Fibers")
  ...
  for (i in seq_along(tissues)) {
    ...
    # Save the plot
    pdf(paste0("results/correlations_macronutrients/heatmaps_v0/heatmap_", tissue, ".pdf"), 
        width = 5, height = 9)
    ...
    write.csv(
      corr_matrix,
      paste0("results/correlations_macronutrients/heatmaps_v0/correlations_", tissue, ".csv")
    )
    
    write.csv(
      p_matrix,
      paste0("results/correlations_macronutrients/heatmaps_v0/pvalues_", tissue, ".csv")
    )
    ...
    pdf(paste0("results/correlations_macronutrients/heatmaps_v0/heatmap_", tissue, "_qvalue.pdf"),
        width = 5, height = 9)
    ...
    write.csv(
      q_matrix,
      paste0("results/correlations_macronutrients/heatmaps_v0/qvalues_", tissue, ".csv")
    )
  }
  ...
  pdf("results/correlations_macronutrients/heatmaps_v0/heatmap_all_tissues.pdf", 
      width = 9, 
  ...
  svg("results/correlations_macronutrients/heatmaps_v0/heatmap_all_tissues.svg", 
      width = 6, 
      height = 25)
  draw(ht_all, annotation_legend_list = list(lgd_sig))
  dev.off()
  ...
  write.csv(
    all_corrs,
    "results/correlations_macronutrients/heatmaps_v0/correlations_all_tissues.csv"
  )
  
  write.csv(
    all_pvals,
    "results/correlations_macronutrients/heatmaps_v0/pvalues_all_tissues.csv"
  )
  ...
  pdf("results/correlations_macronutrients/heatmaps_v0/heatmap_all_tissues_qvalue.pdf",
      width = 9,
      height = max(15, nrow(all_corrs) * 0.25) + 2)
  draw(ht_all_q, annotation_legend_list = list(lgd_sig_q))
  dev.off()

  svg("results/correlations_macronutrients/heatmaps_v0/heatmap_all_tissues_qvalue.svg",
      width = 6,
      height = 25)
  draw(ht_all_q, annotation_legend_list = list(lgd_sig_q))
  dev.off()
  ...
  write.csv(
    all_qvals,
    "results/correlations_macronutrients/heatmaps_v0/qvalues_all_tissues.csv"
  )
}
...
main <- function() {
  ...
  if (length(all_results) > 0) {
    ...
    cat("Heatmaps saved in results/correlations_macronutrients/heatmaps_v0/\n")
  } else {
