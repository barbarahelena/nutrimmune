...
create_complex_heatmaps <- function(all_results) {
  ...
  for (i in seq_along(tissues)) {
    ...
    pdf(paste0("results/correlations/heatmaps_v0/heatmap_", tissue, "_qvalue.pdf"),
        width = 5, height = 9)
    ...
    write.csv(
      q_matrix,
      paste0("results/correlations/heatmaps_v0/qvalues_", tissue, ".csv")
    )
  }
  ...
  pdf("results/correlations/heatmaps_v0/heatmap_all_tissues_qvalue.pdf",
      width = 9,
      height = max(15, nrow(all_corrs) * 0.25) + 2)
  draw(ht_all_q, annotation_legend_list = list(lgd_sig_q))
  dev.off()

  svg("results/correlations/heatmaps_v0/heatmap_all_tissues_qvalue.svg",
      width = 6,
      height = 25)
  draw(ht_all_q, annotation_legend_list = list(lgd_sig_q))
  dev.off()
  ...
  write.csv(
    all_qvals,
    "results/correlations/heatmaps_v0/qvalues_all_tissues.csv"
  )
}
